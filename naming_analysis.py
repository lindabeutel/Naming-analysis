...
def initialize_project():
    """
    Asks the user for the book name, prepares project-specific JSON paths,
    and loads any existing progress data and missing namings if available.
    Returns a tuple: (book_name, last_processed_verse, paths_dict)
    """

    book_name = input("Which book are we working on today? (e.g., Eneasroman): ").strip()
    book_name = book_name[0].upper() + book_name[1:]  # Capitalize first letter

    # Create directory
    os.makedirs("data", exist_ok=True)

    # Prepare paths (book-specific)
    missing_namings_path = os.path.join("data", f"missing_namings_{book_name}.json")
    progress_path = os.path.join("data", f"progress_{book_name}.json")
    collocations_path = os.path.join("data", f"collocations_{book_name}.json")
    categorization_path = os.path.join("data", f"categorization_{book_name}.json")

    # Prepare paths (global)

    lemma_normalization_path = os.path.join("data", f"lemma_normalization.json")
    ignored_lemmas_path = os.path.join("data", f"ignored_lemmas.json")
    lemma_categories_path = os.path.join("data", f"lemma_categories.json")

    paths = {
        "missing_namings_json": missing_namings_path,
        "progress_json": progress_path,
        "collocations_json": collocations_path,
        "categorization_json": categorization_path,
        "lemma_normalization_json": lemma_normalization_path,
        "ignored_lemmas_json": ignored_lemmas_path,
        "lemma_categories_json": lemma_categories_path
    }

    # Load progress or set all to 0
    ...
    return book_name, namings_last_verse, collocations_last_verse, categorization_last_verse, paths
...
def load_data() -> DataType:
    """Interactively asks for Excel and TEI files, loads them if confirmed, and returns them bundled."""
    root = tk.Tk()
    root.withdraw()
    ...
    excel_response = input("Do you want to load an Excel file with existing naming data? (y/n): ").strip().lower()
    if excel_response == "y":
        excel_path = filedialog.askopenfilename(
            title="Select the Excel file with namings",
        ...
        if excel_path:
            while True:
                try:
                    data["excel"] = pd.read_excel(excel_path)
                    data["excel"] = check_required_columns(data["excel"])
    ...
    elif excel_response == "n":
        new_excel = input("Would you like to create a new Excel file instead? (y/n): ").strip().lower()
        if new_excel == "y":
            ...
            if save_path:
                try:
                    ...
                    wb.save(save_path)
                    data["excel"] = pd.read_excel(save_path)
                    data["excel"] = check_required_columns(data["excel"])
    ...
    # 2. Load TEI-XML file?
    xml_response = input("Do you want to load the corresponding TEI file? (y/n): ").strip().lower()
    if xml_response == "y":
        ...
        if xml_path:
            try:
                ...
                root = tree.getroot()
                root = normalize_tei_text(root)
                data["xml"] = root
                print(f"✅ XML file loaded: {os.path.basename(xml_path)}")
            except Exception as e:
                print(f"❌ Error while loading the XML file: {e}")
        else:
...
def search_tei_with_dict(
    ...
    if root is None or df is None or naming_dict is None:
    ...
    for line in verse[start_index:]:
        ...
        if perform_categorization:
            ...
            for entry in entries:
                if int(entry.get("Vers", -1)) != verse_number:
                    continue

                source_text = normalize_text(
                    entry.get("Erzähler") or entry.get("Bezeichnung") or entry.get("Eigennennung") or ""
                )
                if not source_text:
                    continue
                ...
                for e in categorized_entries:
                    ...
                    target_text = normalize_text(
                        e.get("Erzähler") or e.get("Bezeichnung") or e.get("Eigennennung") or ""
                    )
...
def check_and_extend_namings(
    ...
    for naming in dict_namings:
        ...
        # skip if already handled in Excel
        if any(naming in entry for entry in existing_namings):
            continue
        ...
        named_entity = input("Enter the \"Named Entity\": ").strip()
        naming_entity = ""
        if choice == "2":
            naming_entity = input("Enter the \"Naming Entity\": ").strip()
...
def lemmatize_and_categorize_entry(entry, lemma_normalization, paths, ignored_lemmas=None, lemma_categories=None):
    ...
    if ignored_lemmas is None:
        ignored_lemmas = load_ignored_lemmas()

    if lemma_categories is None:
        lemma_categories = load_lemma_categories()
    ...
    if missing:
        while True:
            print(f"\n▶ Please add lemmata for {', '.join(missing)} (comma-separated):")
        ...
        save_lemma_normalization(lemma_normalization)
    ...
    while i < len(lemmata):
        ...
        if user_input == "<":
            ...
            if last_action["type"] == "a":
                ...
                save_ignored_lemmas(ignored_lemmas)
    ...
    if not designations and not epithets:
        ...
        if confirm == "y":
            ...
            updated_entry = lemmatize_and_categorize_entry(entry, lemma_normalization, paths, ignored_lemmas,
                                                           lemma_categories)
    ...
    print("✅ Entry automatically saved.\n")
...
def main():
    ...
    check_namings = input("Should namings be checked and added? (y/n): ").strip().lower() == "y"
    fill_collocations = input("Should empty collocations be filled? (y/n): ").strip().lower() == "y"
    do_categorization = input("Should namings be lemmatized and categorized? (y/n): ").strip().lower() == "y"
