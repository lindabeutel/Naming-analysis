...
def lade_oder_erweitere_benennungen_dict():
    ...
    while erweitern == "j":
        ...
        try:
            ...
            print(f"ℹ daten-Typ: {type(daten)}")
...
def pruefe_benennungen_und_kollokationen(daten, benennungsliste, fehlende_benennungen, letzter_bearbeiteter_vers,normalisierung_dict):
    ...
    for index, row in df.iterrows():
        ...
        vers_text_lower = vers_text.lower()

        # 🔍 Schleife: Alle Zeilen mit diesem Vers prüfen (nicht nur eine!)
        zeilen_selben_verses = df[df["vers"] == vers_nr]

        for zeilen_index, z in zeilen_selben_verses.iterrows():
            vorhandene_benennung = str(z.get("eigennennung", "")).strip() or \
                                   str(z.get("bezeichnung", "")).strip() or \
                                   str(z.get("erzähler", "")).strip()

            if vorhandene_benennung and not str(z.get("kollokationen", "")).strip():
                pattern = r"\b" + re.escape(vorhandene_benennung.lower()) + r"\b"
                if re.search(pattern, vers_text_lower):
                    print("\n-------------------------------------------------------")
                    ...
                    # 📥 Speichere Kollokationen in **konkreter Zeile**
                    df.at[zeilen_index, "kollokationen"] = koll_text

                    # 🧠 Starte Normalisierung/Kategorisierung
                    eintrag = {
                        "Vers": vers_nr,
                        "Benannte Figur": z.get("benannte figur", ""),
                        "Eigennennung": z.get("eigennennung", ""),
                        "Bezeichnung": z.get("bezeichnung", ""),
                        "Erzähler": z.get("erzähler", "")
                    }
        ...
        for name in benennungsliste:
            ...
            if not re.search(pattern, vers_text_lower):
                continue
            ...
            normalisiertes_wort = normalisiere(name, normalisierung_dict)
            ...
            # 🔹 KOLLOKATION bei NEUER Benennung
...
def normalisiere(token: str, normalisierung_dict: dict) -> str:
    return normalisierung_dict.get(token.lower(), token.lower())
...
def finde_vers_im_tei(root, versnummer):
    for elem in root.iter():
        if elem.tag.endswith("l") and elem.get("n") == str(versnummer):
            return elem.text
    return "(Kein Vers gefunden im TEI)"

def extrahiere_benennung(eintrag: dict) -> str:
    for feld in ["bezeichnung", "erzähler", "eigennennung"]:
...
def extrahiere_benennung_aus_tei(tei_text: str, benennungsliste: list) -> str:
    """
    Gibt die erste passende Benennung aus der benennungsliste zurück,
    die im TEI-Text vorkommt (case-insensitive, wortbasiert).
    """
    tei_text = tei_text.lower()
    for benennung in benennungsliste:
        pattern = r"\b" + re.escape(benennung.lower()) + r"\b"
        if re.search(pattern, tei_text):
            return benennung
    return ""
...
def normalisiere_und_kategorisiere_eintrag(entry, normalisierung_dict, ignore_lemmas=None, lemma_kategorien=None):
    ...
    lemmata = [normalisiere(t, normalisierung_dict) for t in tokens]
...
def main():
    ...
    # 🔹 Lade globales Benennungsverzeichnis
    ...
    # 🔹 Excel & TEI laden
    datenpaket = lade_daten()
    df = datenpaket["excel"]
    tei_root = datenpaket["xml"]
    daten = df.to_dict(orient="records")

    # 🔹 Interaktive Entscheidung
    pruefen = input("Möchtest du die Benennungen und leeren Kollokationen auf Basis der TEI-Datei prüfen? (j/n): ").strip().lower()
    normalisierung_dict = {}
    ...
    if pruefen == "n":
        ...
        # 🔁 Jetzt die eigentliche versweise Verarbeitung
        verarbeite_eintraege(daten, tei_root, normalisierung_dict)
